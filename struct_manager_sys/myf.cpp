...
table *memRows(table *tbEdit, char *columns, char *rowName)
{
    ...
    {
        // Обработка ошибки при перераспределении
    ...
    {
        // Обработка ошибки при выделении памяти для новой строки
    ...
    for (int j = 0; j < tbEdit->columnsCount; j++)
    {
        ...
        {
            // Обработка ошибки при выделении памяти для ячейки
...
{
    // нахождение столбцов на
    //*(db->tables[position]->valColumns[0]
    // строки на
    // db->tables[position]->valRow[0] ...
    printf("\033[1;36m====================================\033[0m\n");
    printf("\033[1;32m  Таблица: \033[1;34m%s\033[0m\n", db->tables[position]->nameTb);
    printf("\033[1;36m====================================\033[0m\n");
    printf("\033[1;33mСтолбцы:\033[0m\n");
    for (int i = 0; i < db->tables[position]->columnsCount; i++)
    {
        printf("\033[1;34m| %s \033[0m", db->tables[position]->valColumns[i]);
    }
    printf("\n\033[1;36m------------------------------------\033[0m\n");
    printf("\033[1;33mДанные:\033[0m\n");
    for (int i = 0; i < db->tables[position]->rowCount; i++)
    {
        for (int j = 0; j < db->tables[position]->columnsCount; j++)
        {
            printf("\033[1;32m| %s \033[0m", db->tables[position]->valRow[i][j]);
        }
        printf("\n");
    }
    printf("\033[1;36m====================================\033[0m\n");
...
void databaseControl(database *db)
{
    while (1)
    {
        ...
        {
            printf("Список созданных таблиц с структуре данных %s :\n", db->nameDb);
            for (int i = 0; i < db->tableCount; i++)
                printf("%s\n", db->tables[i]->nameTb);
        }
        ...
        else if (strncmp(command, "CREATE TABLE", 12) == 0)
        {
            ...
            tableName = (char *)malloc(MaxStringSize * sizeof(char));
            columns = (char *)malloc(MaxStringSize * DefMaxColumns * sizeof(char));
            sscanf(command, "CREATE TABLE %s (%[^)])", tableName, columns);
...
void start(database *db)
{
    while (1)
    {
        ...
        else if (strncmp(command, "cd", 2) == 0)
        {
            ...
            sscanf(command, "cd %s", databaseName1);
        ...
        {
            printf("Список созданных структур данных :\n");
            printf("%s\n", db->nameDb);
        }
